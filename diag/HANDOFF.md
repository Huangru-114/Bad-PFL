...
## 5d. Stage B 标定定案（2026-09-09）：**45 步 / 200 轮**

FedBN、100 端 / 选 10 / Nm=4 / ρ=0.1 / 150 轮 / seed 0，唯一自变量 = `local_steps`：

| steps | epoch | MTA_pb | ACC_loc | ASR_filt | ASR_unfilt | s/round |
|---|---|---|---|---|---|---|
| 15 | 1 | 0.5081 | 0.6340 | 0.2186 | 0.2967 | 1.91 |
| **45** | **3** | **0.6536** | **0.7583** | **0.8001** | **0.8201** | 4.73 |
| 75 | 5 | 0.6452 | 0.7639 | 0.8831 | 0.8948 | 7.59 |

**`local_steps = 45`**：MTA 在 3 epoch 到顶，5 epoch 不再改善（0.6452）却贵 41%
→ 取达到平台的最便宜那一档。1 epoch 的 MTA 只有 0.5081，CIFAR-10 上不是能做
后门研究的工作点 —— 导师意见 #5 问的正是这个。

**`total_round = 200`**：45 步下 ASR 约第 140 轮到位（100 轮时 0.6475 vs 终值
0.8001，差 0.15），所以**不能按等算力砍到 100 轮** —— 聚合次数是独立自变量。
200 轮留出 ≥60 轮平坦尾巴，平台判据才判得出来。
成本 0.39 h/run，116 run = **45 GPU-h**。

> **我在这里错过一次，记下来**：我曾拿探针 A（15 步 / **Nm=10, ρ=0.2** / 300 轮，
> MTA 0.6412）去论证「15 步的 MTA 没短板」，而标定是 **Nm=4, ρ=0.1 / 150 轮**
> —— 跨剂量跨轮数的比较，不成立。受控的比较只有标定表本身：同 150 轮同剂量下
> 1 epoch 0.5081 vs 3 epoch 0.6536。
> 我还断言过「45 步会压掉剂量轴的动态范围」—— 那只有 (Nm=4, ρ=0.1) **一个**
> 剂量点的支持，**没有证据，已撤回**。（不过这仍是读结果时值得留意的一件事：
> 若 Nm ∈ {4,8,16,32} 的格子都挤在 0.8–0.95，剂量–响应就压平了。
> ρ=0 与 Nm=1 两格给出下界，看那两格就知道。）

**仍未确定**：150 轮处三档的平台**都没判出来**（全部落在尾窗）。
200 轮能不能判出来，要等主力跑完才知道。
